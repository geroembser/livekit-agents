...
def llm_stream():
    mock_llm = MagicMock()
    mock_llm._thought_signature_storage = InMemoryThoughtSignatureStorage()
...
class TestThoughtSignatureStorage:
    @staticmethod
    ...
    def _stream(
        ...
        async def generate() -> AsyncIterator[types.GenerateContentResponse]:
            yield response

        return generate()

    @pytest.mark.asyncio
    async def test_gemini_3_parallel_calls_share_one_turn_signature(self) -> None:
        storage = InMemoryThoughtSignatureStorage()
        primary = LLM(
            model="gemini-3-flash-preview",
            api_key="test",
            thought_signature_storage=storage,
        )
        fallback = LLM(
            model="gemini-3-flash-preview",
        ...
        response = self._response(
            [
                types.Part(
                    function_call=types.FunctionCall(id="call-1", name="first", args={}),
                    thought_signature=b"turn-signature",
                ),
                types.Part(function_call=types.FunctionCall(id="call-2", name="second", args={})),
            ]
        )

        async def fake_stream(**_kwargs: Any) -> AsyncIterator[types.GenerateContentResponse]:
            return self._stream(response)

        with patch.object(
            primary._client.aio.models,
            "generate_content_stream",
            AsyncMock(side_effect=fake_stream),
        ):
            stream = primary.chat(chat_ctx=ChatContext.empty())
            try:
        ...
        assert primary.thought_signature_storage is fallback.thought_signature_storage
        assert storage.get_all() == {
            "call-1": b"turn-signature",
            "call-2": b"turn-signature",
        }

    def test_gemini_25_stores_its_function_call_signature(self) -> None:
        storage = InMemoryThoughtSignatureStorage()
        mock_llm = MagicMock()
        mock_llm._thought_signature_storage = storage

        with patch.object(LLMStream, "__init__", lambda self, *a, **kw: None):
            stream = LLMStream.__new__(LLMStream)
            stream._llm = mock_llm
            stream._model = "gemini-2.5-flash"

        stream._parse_part(
            "request-id",
            types.Part(
                function_call=types.FunctionCall(id="call-25", name="tool", args={}),
                thought_signature=b"gemini-25-signature",
            ),
        )

        assert storage.get_all() == {"call-25": b"gemini-25-signature"}
...
class TestGatewayOptions:
    def test_gateway_preserves_new_livekit_options(self) -> None:
        ...
        assert model.thought_signature_storage is storage
        assert model._opts.service_tier == types.ServiceTier.PRIORITY
