...
class ThoughtSignatureStorage(ABC):
    ...
    Thought signatures are used by Gemini 2.5+ models for multi-turn function calling
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            thought_signature_storage (ThoughtSignatureStorage, optional): Custom storage for Gemini 2.5+ thought signatures.
    ...
    def with_gateway(
        ...
        *,
        ...
        Args:
            ...
            service_tier (types.ServiceTier, optional): The service tier for the request.
            cached_content (str, optional): Resource name of an explicit context cache.
            media_resolution (types.MediaResolution, optional): The media resolution for the request.
...
class LLMStream(llm.LLMStream):
    ...
    async def _run(self) -> None:
        ...
        try:
            ...
            async for response in stream:
                ...
                # Gemini 3 puts one signature on the first call of a parallel call set.
                turn_thought_signature: bytes | None = None
                if _is_gemini_3_model(self._model):
                    turn_thought_signature = next(
                        (
                ...
                for part in candidate.content.parts:
                    chat_chunk = self._parse_part(
                        request_id,
                        part,
                        turn_thought_signature=turn_thought_signature,
                    )
    ...
    def _parse_part(
        ...
        if part.function_call:
            ...
            # Gemini 3 returns one signature for a parallel call set. Each call ID must
            # map to that signature because a later fallback client can process any call.
            effective_thought_signature = (
                part.thought_signature
                if hasattr(part, "thought_signature") and part.thought_signature
                else turn_thought_signature
            )
            if _requires_thought_signatures(self._model) and effective_thought_signature:
